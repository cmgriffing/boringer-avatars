<template>
    ...
    <template :key="index" v-for="(variantOption, index) in variants">
      <label  :class="_classStringToObject(`radio-label ${variantOption === selectedVariant ? 'selected' : ''}`)"  :for="`radio-${variantOption}`" >
          <input  type="radio"  class="radio-item sr-only"  name="variant-option"  :id="`radio-${variantOption}`"  :value="variantOption"  @input="handleChange($event)"  />
          {{variantOption}}
        </label>
    </template>
  </div>
  ...
  import { coerceVariant, AvatarVariant, variants } from '../utils/common.utils';
      ...
      watch: {
          onUpdateHook0: { handler() { this.selectedVariant = coerceVariant(this.variant) || "beam" }, immediate: true }
        },
